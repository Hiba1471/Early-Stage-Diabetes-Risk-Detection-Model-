...
I calculated Youden's J index, $J = \text{Sensitivity} + \text{Specificity} - 1$, across the ROC curve. This is a common healthcare evaluation approach and helped identify the optimal threshold for each model by balancing Recall and Specificity.
...
I generated SHAP (SHapley Additive exPlanations) values using \texttt{TreeExplainer} on the LightGBM model. This identified which features influenced each individual prediction the most and made the model's predictions more readable and explainable.
