...
**Author:** Hiba Khan  
**Date:** May 4, 2026  
**Focus:** Non-invasive diabetes-risk prediction, recall-oriented evaluation, and model interpretability
...
| Positive class prevalence | 13.93% diabetic |
| Primary evaluation priority | Recall / Sensitivity |
| Final practical model choice | LightGBM |
| Optimized recall | 81%+ |
...
- [2. Data Explanation and Exploration](#2-data-explanation-and-exploration)
  - [EDA and Assumption Checks](#eda-and-assumption-checks)
  - [Preprocessing Strategy](#preprocessing-strategy)
- [3. Algorithms and Techniques Analysis](#3-algorithms-and-techniques-analysis)
  - [Hyperparameter Tuning and Class Imbalance](#hyperparameter-tuning-and-class-imbalance)
  - [Initial Results at Default 0.5 Threshold](#initial-results-at-default-05-threshold)
  - [Threshold Optimization with Youden's J Statistic](#threshold-optimization-with-youdens-j-statistic)
...
Diabetes is one of the leading causes of immense financial pressure on international healthcare support systems, both individual and global. The prevalence of this chronic illness needs no introduction as cases keep surging with each passing year, up from 200 million in 1990 to 830 million in 2022. This is mostly caused by increasing obesity, sedentary careers, and more people aging than ever before.

If we manage to detect this chronic public health crisis early, we can avoid or at least mitigate many of the severe complications like end-stage renal disease, cardiovascular disease, and neuropathy.
...
I performed assumption checks before constructing the models. Based on prior knowledge, multiple relationships were expected: inactive physical life correlates with increased BMI, which is expected to influence diabetes risk; similarly, reduced healthcare access is often associated with lower income.

To detect potential multicollinearity among features such as general health, mental health days, and BMI, I generated a Pearson correlation matrix and calculated Variance Inflation Factors (VIF) for all predictors. The results showed that multicollinearity was not a limiting factor for linear models because VIF values stayed within acceptable bounds of 5.
...
To combat the 6:1 class imbalance, I implemented algorithmic penalization instead of synthetic resampling such as SMOTE. SMOTE added computational time and complexity without improving performance, especially since the dataset is large.

Options such as `class_weight='balanced'` and `is_unbalance=True` were specified. Models were optimized with 3- to 5-fold `GridSearchCV`, using Recall as the primary scoring metric to prioritize catching true diabetes cases.
...
I calculated Youden's J index:

```text
J = Sensitivity + Specificity - 1
```

This was calculated across the ROC curve. This is a common healthcare evaluation approach and helped identify the optimal threshold for each model by balancing Recall and Specificity.
...
![Calibration, Precision-Recall, and ROC curves](assets/figure-1.png)

**Figure 1.** Calibration, Precision-Recall, and ROC curves for benchmark models. LightGBM matches the Ensemble across major metrics while remaining better calibrated.
...
## 4. Why the Ensemble Did Not Outperform
...
### Reason 1: Probability Calibration Divergence
...
### Reason 2: Feature Space Redundancy
...
### Reason 3: The Equalizing Power of Youden's J
...
I generated SHAP (`SHapley Additive exPlanations`) values using `TreeExplainer` on the LightGBM model. This identified which features influenced each individual prediction the most and made the model's predictions more readable and explainable.

![Global SHAP plot showing top features](assets/figure-2.png)
...
![Individual SHAP waterfall plot](assets/figure-3.png)
...
To make the images display correctly on GitHub, upload this Markdown file together with the `assets/` folder:
